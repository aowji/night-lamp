...
#else
#define EXAMPLE_DPP_DEVICE_INFO ""
#endif
...
static bool s_dpp_enrollee_enabled = false;

static void event_handler(void *arg, esp_event_base_t event_base,
                          int32_t event_id, void *event_data) {
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        if (s_dpp_enrollee_enabled) {
...
esp_err_t dpp_enrollee_bootstrap(void) {
    ...
    if (pkey_len) {
        /* Currently only NIST P-256 curve is supported, add prefix/postfix
         * accordingly */
        char prefix[] = "30310201010420";
        char postfix[] = "a00a06082a8648ce3d030107";

        if (pkey_len != CURVE_SEC256R1_PKEY_HEX_DIGITS) {
            ESP_LOGI(TAG, "Invalid key length! Private key needs to be 32 "
                          "bytes (or 64 hex digits) long");
            return ESP_FAIL;
        }
    ...
    ret = esp_supp_dpp_bootstrap_gen(EXAMPLE_DPP_LISTEN_CHANNEL_LIST,
                                     DPP_BOOTSTRAP_QR_CODE, key,
...
void dpp_enrollee_init(void) {
    ...
    if (s_dpp_wifi_config.sta.ssid[0] != '\0') {
        ESP_LOGI(TAG, "Wi-Fi has been configured, connecting to the AP");
        ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
        ESP_ERROR_CHECK(esp_wifi_start());
    } else {
        ESP_LOGI(TAG, "Starting DPP Enrollee");
        s_dpp_enrollee_enabled = true;
        ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
    ...
    if (bits & DPP_CONNECTED_BIT) {
        ...
        ESP_LOGI(TAG, "DPP Authentication failed after %d retries",
                 s_retry_num);
    } else {
    ...
    // ESP_EVENT_ANY_ID, &event_handler)); vEventGroupDelete(s_dpp_event_group);
}
